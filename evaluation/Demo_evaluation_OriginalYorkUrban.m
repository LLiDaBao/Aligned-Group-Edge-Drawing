...
name_method = {'\fontname{times}LSD', '\fontname{times}EDLines', '\fontname{times}Linelet_H', '\fontname{times}Linelet', '\fontname{times}AG3Line',...
    '\fontname{times}ELSED', '\fontname{times}my'};
method2test = {'lsd', 'edlines', 'linelet', 'linelet', 'ag3line', 'elsed', 'my'};
...
dir_method = {'lsd','edlines','proposed_h', 'proposed','ag3line', 'elsed', 'my'};
var_method = {'line_own', 'line_own', 'line_own', 'line_own','line_own','line_own','line_own'};
...
eval_param.thres_length_ratio = .5; % .75 (Con1) or .5 (Con2)
    ...
    try
        ...
        for k = 1:num_method
            ...
            if strcmp(dir_method{k}, 'proposed') || strcmp(dir_method{k}, 'proposed_h')
                ...
                str_est = sprintf('result/%s/%s.csv', dir_method{k}, Image_ID_List(i_im).name);
            end
            ...
            elseif strcmp(dir_method{k}, 'proposed_h')
                ...
                line_own = [segments center length angle];
            end
